...
class TradeController extends Controller {
    ...
    public function closePosition(){
        ...
        ///
        foreach ($list as $v){
            $goodsIdArr[] = $v['code_id'];
        }

        $whereIds['id'] = array('in',array_unique($goodsIdArr));
        $goods = M('actuals_goods')->where($whereIds)->select();
        ...
        foreach ($list as $key=>$value){
            ...
            $list[$key]['actaulInfo'] = $goodInfo[$value['code_id']];
    ...
    public function openPosition(){
        ...
        $whereIds['id'] = array('in',$goodsIdArr);
        $goods = M('actuals_goods')->where($whereIds)->select();
        ...
        foreach ($list as $key=>$value){
            $list[$key]['actaulInfo'] = $goodInfo[$value['code_id']];
            $list[$key]['close_position_time'] = date("Y-m-d H:i:s",$value['close_position_time']);
            $list[$key]['tid'] = $value['position_id'];
        }
    ...
    public function report(){
        ...
        $uidArr = $this->getUserInfoUid($usermap);

        $whereUids = $map['uid'] = array('in',$uidArr);
...
//        $userRows = M('user_info')->where($userMap)->find();
        ...
        //
        $whereRowIdArr['uid'] = $whereUids;
        $userRows = M('user_info')->where($whereRowIdArr)->select();
        foreach ($userRows as $ku=>$u){
            $userInfoRow[$u['uid']] = $u;
        }
        //
        ...
        foreach ($list as $key=>$value){
            $list[$key]['actaulInfo'] = $goodInfo[$value['code_id']];
            $list[$key]['userInfo'] = $userInfoRow[$value['uid']];
            $list[$key]['close_position_time'] = date("Y-m-d H:i:s", $value['close_position_time']);
    ...
    public function getUserInfoUid($userWhere=array()){
        //权限设置 访问

        $identity_id = $this->user['identity_id'];

        if($identity_id==2){ //机构访问
            $userWhere['memberId'] = $this->user['memberId'];
        }else if($identity_id==3){// 经纪人访问
            $userWhere['memberId'] = $this->user['memberId'];
            $userWhere['agentId'] = $this->user['agentId'];
        }
...
//        $uidArr = array();
//        if($memberId) {
//            $userWhere['memberId'] = $memberId; //现在只考虑机构
//        }

        $userRow = M('user_info')->where($userWhere)->select();
        $uidArr = array();
        foreach($userRow as $u){
            $uidArr[] = $u['uid'];
        }
